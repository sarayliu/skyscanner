...
function AirportInfo() { 
    ...
    function handleSubmit(e) {
        ...
        async function fetchMyAPI() {
            ...
            let response = await fetch("https://skyscanner-skyscanner-flight-search-v1.p.rapidapi.com/apiservices/browsedates/v1.0/US/USD/en-US/" + new URLSearchParams(qString), reqOptions)
            ...
            // let response = await fetch("https://skyscanner-skyscanner-flight-search-v1.p.rapidapi.com/apiservices/browsequotes/v1.0/US/USD/en-US/?" +  originplace + "/" + destinationplace, reqOptions)
    ...
    return(
        <div className="airportinfo">
           <form onSubmit={handleSubmit}>
                ...
                <input id="queryInput" value={originplace} onChange={e => setOriginplace(e.target.value)} required/>
                <label htmlFor="queryInput">To:</label>
                <input id="queryInput" value={destinationplace} onChange={e => setDestinationplace(e.target.value)} required/>
                <br />
                <label htmlFor="queryInput">Depart:</label>
                <input id="queryInput" value={outboundpartialdate} onChange={e => setOutboundpartialdate(e.target.value)} required/>
                <label htmlFor="queryInput">Arrive:</label>
                <input id="queryInput" value={inboundpartialdate} onChange={e => setInboundpartialdate(e.target.value)} required/>
